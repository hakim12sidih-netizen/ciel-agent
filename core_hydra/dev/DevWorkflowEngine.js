...
export class DevWorkflowEngine {
    ...
    cwd;
    maxIterations = 5;
    setMaxIterations(n) { this.maxIterations = n; }
    constructor(engine, cwd = process.cwd()) {
        this.engine = engine;
        this.cwd = cwd;
    ...
    async run(task) {
        ...
        try {
            ...
            steps.push(readStep);
            if (!readStep.success)
                throw new Error('Codebase reading failed.');
            ...
            if (!testStep.success) {
                const corrStep = await this.phaseCorrect(task, planStep.output, testStep.output);
                steps.push(corrStep);
    ...
    // ══════════════════════════════════════════════════════════════════════════
    async phaseReadCodebase(task) {
        const start = Date.now();
        logger.info('[DevWorkflow] 📖 Phase 1: Reading codebase...');
        try {
            // Get a structural snapshot of relevant source files
            const { stdout: gitFiles } = await execAsync('git ls-files --cached --others --exclude-standard', { cwd: this.cwd });
...
Select the TOP 10 files most relevant to this task. Return ONLY a JSON array of filenames. No explanation.`;
            ...
            for await (const chunk of this.engine.query(relevancePrompt)) {
                if (chunk.type === 'text')
                    relevantFilesJson += chunk.content;
            }
            let relevantFiles = [];
            try {
    ...
    async phasePlan(task, codebaseContext) {
        const start = Date.now();
...
Task: ${task}`;
            ...
            for (const cmd of commands.slice(0, 8)) {
                ...
                try {
                    ...
                    if (stdout)
                        outputs.push(`$ ${cmd.split('\n')[0]}\n${stdout}`);
                    if (stderr)
                        outputs.push(`[STDERR] ${stderr.substring(0, 200)}`);
                }
                catch (e) {
    ...
    async phaseTest() {
        ...
        for (const cmd of testCommands) {
            try {
                ...
                results.push(`$ ${cmd}\n${errOut}`);
                if (errOut.includes('error') || errOut.includes('TS'))
                    hasErrors = true;
            }
        }
    ...
    async phaseCorrect(task, plan, testOutput, iteration = 1) {
        const start = Date.now();
...
Be surgical: fix only what's broken.`;
        ...
        return this.step('CORRECTING', `${applied.length} files corrected (iteration ${iteration}).`, applied.join('\n') + '\n\n' + correction.slice(0, 1000), start, applied.length > 0);
    }
    async phaseDeliver(task, executionOutput) {
        const start = Date.now();
        logger.info('[DevWorkflow] 📦 Phase 6: Delivering (PR/commit)...');
        const outputs = [];
        try {
            const { stdout: diffStat } = await execAsync('git diff --stat HEAD', { cwd: this.cwd, timeout: 10000, encoding: 'utf8' }).catch(() => ({ stdout: 'No git repo or no changes.' }));
...
Format: <type>(<scope>): <subject>`;
    ...
    //  HELPERS
    // ══════════════════════════════════════════════════════════════════════════
    step(phase, description, output, startMs, success) {
        const step = {
            phase,
            description,
            output,
    ...
    buildFinalReport(taskId, task, steps) {
        const lines = [
            ...
            '',
            '## Phase Summary',
            ...steps.map(s => `- ${s.success ? '✅' : '❌'} **${s.phase}** — ${s.description} _(${s.durationMs}ms)_`),
        ];
        return lines.join('\n');
    }
}
