...
export class DreamEngine {
  ...
  constructor(
    ...
    private core: HydraCore
  ) {}

  /**
   * Déclenche un cycle complet de Rêve Lucide (Auto-entraînement)
   */
  public async startDreamCycle(durationMinutes: number = 60) {
    ...
    try {
      ...
      const hippocampus = this.brain.getHippocampus() as { consolidate?: () => void };
   ...
   */
  private async runSelfPlayIteration(idx: number) {
  ...
  private generateChallenge(): string {
    const phoenix = (this.core as unknown as { phoenix?: { getFailures?: () => Array<{ mission: string; error: string }> } }).phoenix;
