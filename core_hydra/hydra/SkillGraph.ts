...
import { HydraNerveSystem } from './HydraNerveSystem.js';
...
export interface SkillNode {
  ...
  layer: SkillLayer;
  skill: string;
  toolRef?: string;        // Référence vers le vrai outil Hydra
...
// ════════════════════════════════════════════════════════════
const SKILL_DEFINITIONS: Record<number, { name: string; tool: string }> = {
...
function getLayer(nodeId: number): SkillLayer {
  if (nodeId <= 60) return SkillLayer.PERCEPTION;
  if (nodeId <= 150) return SkillLayer.COGNITION;
  if (nodeId <= 240) return SkillLayer.ACTION;
  if (nodeId <= 300) return SkillLayer.METASYSTEM;
  return SkillLayer.COSMOS;
}

export class SkillGraph {
  private nodes: Map<number, SkillNode> = new Map();
  ...
  private initialize(): void {
    ...
    this.nodes.forEach((node) => {
      for (let offset = 1; offset <= 3; offset++) {
        ...
        if (neighbor <= 360) node.connections.add(neighbor);
      }
    });
   ...
   */
  async loadPythonSkills(): Promise<void> {
    ...
    for (const skill of pythonSkills) {
      ...
      if (skill.layer === 'PERCEPTION') layer = SkillLayer.PERCEPTION;
      if (skill.layer === 'ACTION') layer = SkillLayer.ACTION;
      if (skill.layer === 'METASYSTEM') layer = SkillLayer.METASYSTEM;
      if (skill.layer === 'COSMOS') layer = SkillLayer.COSMOS;
  ...
  activateNodes(nodeIds: number[]): void {
    const now = Date.now();
  ...
  resolveToolRef(nodeId: number): string | undefined {
    return this.nodes.get(nodeId)?.toolRef;
  }

  /** Find all nodes that reference a specific tool */
  findByTool(toolName: string): SkillNode[] {
    const results: SkillNode[] = [];
    this.nodes.forEach(n => { if (n.toolRef === toolName) results.push(n); });
    return results;
  }

  selectNodes(complexity: number, tags: string[]): number[] {
    const selected: number[] = [...Array(60)].map((_, i) => i + 1);
    if (complexity >= 4) selected.push(...[...Array(90)].map((_, i) => i + 61));
    if (complexity >= 8) selected.push(...[...Array(90)].map((_, i) => i + 151));
  ...
  getStats(): { total: number; active: number; dormant: number } {
    let active = 0;
    this.nodes.forEach((n) => { if (n.state === AgentMemoryState.ACTIVE) active++; });
    return { total: 360, active, dormant: 360 - active };
  }

  /** Add a new "anti-failure" node (Phoenix protocol) */
  addAntiFailureNode(skill: string): number {
    const newId = this.nodes.size + 1;
    this.nodes.set(newId, {
      id: newId,
