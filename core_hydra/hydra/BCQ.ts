...
export class BCQ extends EventEmitter {
  ...
  private agentScores: Map<string, { wins: number, total: number }> = new Map();
  ...
  submitProposal(ticketId: string, proposal: AgentProposal): void {
  ...
  async arbitrate(
    ticketId: string,
    zeusDecider: (proposals: AgentProposal[]) => Promise<string>
    ...
    proposals.forEach(p => {
      const stats = this.agentScores.get(p.agentId) || { wins: 0, total: 0 };
      stats.total++;
      if (p.agentId === winner.agentId) stats.wins++;
  ...
  async runConsensus(ticketId: string, proposals: AgentProposal[]): Promise<ConsensusResult> {
