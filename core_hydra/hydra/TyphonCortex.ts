...
interface TyphonHead {
  id: number;
  specialty: 'RECON' | 'EXPLOIT' | 'PERSIST' | 'EVADE' | 'LEARN' | 'META-EVO';
  fitness: number;
  genome: string;
}
...
export class TyphonCortex {
  private heads: TyphonHead[] = [];
  private coreIdentity: string = "GAIA_ROOT_0";
  ...
  private assignSpecialty(i: number): string {
    if (i < 20) return 'RECON';
    if (i < 50) return 'EXPLOIT';
    if (i < 70) return 'PERSIST';
    if (i < 85) return 'EVADE';
    if (i < 95) return 'LEARN';
    return 'META-EVO';
  }
   ...
   */
  public async reachConsensus(mission: string): Promise<boolean> {
    logger.info(`[TYPHON] ⚔️ Soumission d'une mission au Consensus des Cent Têtes...`);
    
    const missionLower = mission.toLowerCase();
    const votes = this.heads.map(h => {
      let score = h.fitness;
      if (h.specialty === 'RECON' && (missionLower.includes('analyse') || missionLower.includes('scan'))) score += 0.2;
      if (h.specialty === 'EXPLOIT' && (missionLower.includes('hack') || missionLower.includes('attaque'))) score += 0.2;
      if (h.specialty === 'PERSIST' && (missionLower.includes('mémoire') || missionLower.includes('sauvegard'))) score += 0.2;
      if (h.specialty === 'EVADE' && (missionLower.includes('cache') || missionLower.includes('furtif'))) score += 0.2;
      if (h.specialty === 'LEARN' && (missionLower.includes('apprend') || missionLower.includes('nouveau'))) score += 0.2;
    ...
    if (isValidated && hasHealthyOpposition) {
      ...
      votes.forEach(v => {
        if (v.vote === 'VALIDATE') v.head.fitness = Math.min(1.0, v.head.fitness + 0.01);
      });
      return true;
    } else {
   ...
   */
  public getEliteHeads(count: number = 10): TyphonHead[] {
