...
export class ContextCollector {
    ...
    collectProject() {
        ...
        const ctx = { cwd, projectName: basename(cwd) };
        const tryExec = (cmd) => {
            try {
        ...
        else if (existsSync(resolve(cwd, 'Cargo.toml')))
            ctx.language = 'Rust';
    ...
    formatForPrompt() {
        ...
        if (ctx.project.gitBranch)
            lines.push(`Git: ${ctx.project.gitBranch}`);
        if (ctx.project.language)
            lines.push(`Lang: ${ctx.project.language}`);
        if (ctx.project.framework)
            lines.push(`Framework: ${ctx.project.framework}`);
        if (ctx.project.packageManager)
            lines.push(`PM: ${ctx.project.packageManager}`);
        return lines.join('\n');
    }
}
