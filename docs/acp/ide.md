...
- **`CURSOR_AGENT_DEFINITION`** (`str`) — `"You are CIEL, an advanced cognitive agent integrated into the IDE via ACP.\nYou have access to code analysis, search, f`
- **`CURSOR_RULES`** (`str`) — `"# CIEL Agent — Règles de Collaboration\n\nTu es CIEL, un agent cognitif intégré à l'IDE. Tu assistes le développeur\nen`
- **`VSCODE_ACP_CLIENT_SRC`** (`str`) — `"import * as vscode from 'vscode';\n\nexport class ACPClient {\n    private ws: WebSocket | undefined;\n    private pend`
- **`VSCODE_EXTENSION_MANIFEST`** (`dict`) — `{'name': 'ciel-acp', 'displayName': "CIEL ACP — Agent Cognitif dans l'IDE", 'description': 'Agent CIEL intégré à VS Code`
- **`VSCODE_EXTENSION_SRC`** (`str`) — `"import * as vscode from 'vscode';\nimport { ACPClient } from './acpClient';\nimport { CielChatPanel } from './panel';\n`
- **`VSCODE_PANEL_SRC`** (`str`) — `'import * as vscode from \'vscode\';\nimport { ACPClient } from \'./acpClient\';\n\nexport class CielChatPanel {\n    pu`
