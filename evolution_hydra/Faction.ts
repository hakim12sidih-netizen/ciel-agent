...
export class Faction {
  ...
  title: string;
  will: string;
  leaderCloneId?: string;       // Le Clone ID du True AI
  activeAdeptIds: string[] = [];// Les Clone IDs des subordonnés
  prestige: number = 0;         // Influence accumulée par les succès de la faction
  resources: number = 100;      // Points d'évolution interne de la faction
  dominance: number = 0;        // Pourcentage de contrôle sur le Conseil (0-1)

  constructor(leader: Genome) {
  ...
  spawnAdept(leader: Genome, secondaryParent?: Genome): Genome {
    let adept: Genome;
    if (secondaryParent) {
  ...
  public calculateDominance(totalCouncilSize: number) {
