...
export interface CausalEdge {
  from: string;
  to: string;
  strength: number; // -1 to 1 (negative correlation to positive causation)
}
...
export class CausalBrain {
  ...
  registerEvent(id: string, type: NodeType, description: string) {
    const node: CausalNode = { id, type, description, timestamp: Date.now() };
    this.nodes.set(id, node);
    logger.debug(`[Causal Brain] Event Registered: ${id} (${type})`);
  }

  createCausalLink(fromId: string, toId: string, strength: number = 0.5) {
   ...
   */
  async counterfactualReasoning(targetActionId: string, desiredOutcome: string): Promise<string> {
    const action = this.nodes.get(targetActionId);
    if (!action) return 'Action not found in causal map.';
    ...
    for await (const chunk of this.engine.query(prompt)) {
      if (chunk.type === 'text') reasoning += chunk.content;
    }
