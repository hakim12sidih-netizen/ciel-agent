...
export var EntropySourceType;
(function (EntropySourceType) {
    EntropySourceType["CPU_FLUCTUATION"] = "CPU_FLUCTUATION";
    EntropySourceType["MEMORY_VARIANCE"] = "MEMORY_VARIANCE";
    EntropySourceType["EVENT_LOOP_LAG"] = "EVENT_LOOP_LAG";
    EntropySourceType["TIMING_JITTER"] = "TIMING_JITTER";
    EntropySourceType["MUTATION_DRIFT"] = "MUTATION_DRIFT";
    EntropySourceType["NETWORK_LATENCY"] = "NETWORK_LATENCY";
    EntropySourceType["DISK_IO_VARIANCE"] = "DISK_IO_VARIANCE";
    EntropySourceType["THERMAL_NOISE"] = "THERMAL_NOISE";
})(EntropySourceType || (EntropySourceType = {}));
export var EntropyProfile;
(function (EntropyProfile) {
    EntropyProfile["CHAOTIC"] = "CHAOTIC";
    EntropyProfile["STRUCTURED"] = "STRUCTURED";
    EntropyProfile["RESONANT"] = "RESONANT";
    EntropyProfile["CATALYTIC"] = "CATALYTIC";
    EntropyProfile["DORMANT"] = "DORMANT";
})(EntropyProfile || (EntropyProfile = {}));
// ─── Le Récolteur d'Entropie ────────────────────────────────
export class EntropyHarvester {
    sources = new Map();
    harvested = [];
    entropyPool = []; // Pool d'entropie brute
    state;
    lastHarvestTime = Date.now();
    stochasticResonanceOptimum = 0.3; // Niveau optimal de bruit
    constructor() {
        this.state = {
            totalHarvested: 0,
            totalUsed: 0,
     ...
     */
    generateCreativeMutation(genome) {
     ...
     */
    computeStochasticResonance(systemPerformance) {
     ...
     */
    computeAntifragilityIndex(recentPerformanceHistory) {
        if (recentPerformanceHistory.length < 5)
            return 0.5;
    ...
    assessSourceQuality(type, value) {
    ...
    computeCompositeEntropy(sources) {
    ...
    classifyEntropy(sources, composite) {
        ...
        if (composite > 0.7 && avgQuality > 0.6)
            return EntropyProfile.CATALYTIC;
        if (variance < 0.05 && avgQuality > 0.5)
    ...
    computeCreativePotential(sources, profile) {
    ...
    updateStats(latestHarvest) {
        ...
        for (const [profile, count] of Object.entries(profileCounts)) {
            if (count > maxCount) {
                maxCount = count;
                this.state.dominantProfile = profile;
            }
        }
    }
    // ─── Getters ────────────────────────────────────────────
