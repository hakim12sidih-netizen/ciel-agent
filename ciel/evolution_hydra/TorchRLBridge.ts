...
export interface RLState {
  /** Vecteur d'état 12D. */
  state: number[];
  /** Poids de la policy (matrice 12x12). */
  weights: number[][];
  /** Biais de la policy (vecteur 12). */
  bias: number[];
}

export interface RLStepResult {
  status: 'updated' | 'error' | 'fallback';
  loss?: number;
  iterations?: number;
  weights?: number[][];
  bias?: number[];
...
export interface TorchRLBridgeOptions {
  /** Chemin vers le script Python (default: hydra_vampire_v4/rl_trainer.py). */
  trainerPath?: string;
  /** Chemin du checkpoint (default: /tmp/hydra_rl_checkpoint.json). */
  checkpointPath?: string;
  /** Timeout en ms (default: 30_000). */
  timeoutMs?: number;
  /** Python interpreter (default: python3). */
  pythonBin?: string;
  /** Force le fallback TS (skip le subprocess). Utile pour les tests. */
...
export class TorchRLBridge {
  ...
  constructor(options: TorchRLBridgeOptions = {}) {
   ...
   */
  public async trainStep(state: number[], reward: number[]): Promise<RLStepResult> {
    this.callCount++;
    ...
    try {
      ...
      logger.warn(`[TorchRL] ⚠️ Python trainer failed (${(err as Error).message}), using TS fallback`);
      this.fallbackCount++;
      return this.fallbackTrainStep(state, reward);
    }
  }
   ...
   */
  public async trainStepWithMock(
    state: number[],
    reward: number[],
    mockFn: (input: { state: number[]; reward: number[] }) => Promise<RLStepResult>
  ): Promise<RLStepResult> {
    this.callCount++;
    try {
   ...
   */
  public loadCheckpoint(): RLState | null {
    ...
    try {
      ...
      logger.warn(`[TorchRL] Could not load checkpoint: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  private async runTrainer(input: { state: number[]; reward: number[] }): Promise<RLStepResult> {
    return new Promise<RLStepResult>((resolve, reject) => {
      ...
      proc.on('close', (code) => {
        ...
        try {
          const result = JSON.parse(lastLine) as RLStepResult;
          resolve(result);
        } catch (err) {
   ...
   */
  private async fallbackTrainStep(state: number[], reward: number[]): Promise<RLStepResult> {
    const ckpt = this.loadCheckpoint();
    let W: number[][] = ckpt?.weights ?? new Array(12).fill(null).map(() => new Array(12).fill(0));
    let b: number[] = ckpt?.bias ?? new Array(12).fill(0);
    ...
    try {
      fs.writeFileSync(
        this.checkpointPath,
        JSON.stringify({ weights: W, bias: b, state, iterations: (ckpt?.bias ? 1 : 0) + 1 })
      );
    } catch (err) {
      logger.warn(`[TorchRL] Could not save checkpoint: ${(err as Error).message}`);
    }
