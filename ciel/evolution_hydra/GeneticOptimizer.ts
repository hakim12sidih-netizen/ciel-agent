...
import { UnifiedGenome, ChromosomeType } from './UnifiedGenome.js';
...
/**
 * Profil de personnalité simplifié (6 traits).
 * Dérivé d'un UnifiedGenome, utilisé pour la dynamique de faction.
 * Conservé pour rétro-compatibilité avec le code existant.
 */
 ...
 */
export function unifiedToPersonality(g: UnifiedGenome): PersonalityProfile {
...
export class GeneticOptimizer {
  private agentGenomes: Map<string, PersonalityProfile> = new Map();

  constructor() {
    this.initializeGenomes();
  }

  private initializeGenomes() {
   ...
   */
  public async absorbInto(survivor: UnifiedGenome, victim: UnifiedGenome): Promise<void> {
    if (!survivor || !victim) return;
    // Transférer 1 gène aléatoire du chromosome BEHAVIOR
    const victimGenes = victim.g_behavior;
    if (victimGenes.length === 0) return;
   ...
   */
  public generateHeirsFrom(chief: UnifiedGenome, partner: UnifiedGenome): UnifiedGenome[] {
    if (!chief || !partner) return [];
    logger.info(`[DYNASTY] 👑 Le Chef ${chief.id} engendre sa lignée avec ${partner.id}`);

    const heirs: UnifiedGenome[] = [];
    for (let i = 0; i < 2; i++) {
   ...
   */
  public generateHeirs(chiefId: string, partnerId: string): PersonalityProfile[] {
    ...
    const heirs: PersonalityProfile[] = [];
    for (let i = 0; i < 2; i++) {
      const heir: PersonalityProfile = {
   ...
   */
  public electPatron(faction: Faction, averageGenome: PersonalityProfile): string {
   ...
   */
  public async mutateFaction(
    faction: Faction,
    trait: keyof PersonalityProfile,
    intensity: number,
    population: UnifiedGenome[] = []
  ): Promise<{ status: 'applied'; count: number; faction: Faction }> {
    ...
    for (const genome of population) {
      ...
      await genome.mutateByStrategy(
        Math.floor(Math.random() * genome.g_behavior.length),
  ...
  private pickStrategyForFaction(faction: Faction): 'gaussian' | 'epigenetic' | 'transposon' | 'bricolage' | 'sacrifice' {
    switch (faction) {
