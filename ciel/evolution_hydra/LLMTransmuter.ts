...
import {
  MetamorphicCore,
  type ArchitecturalGene,
  type TransmutationProposal,
} from './MetamorphicCore.js';
import { TransmutationBudget, type BudgetCheck } from './TransmutationBudget.js';

export type TransmutationStatus =
  | 'rejected_by_budget'
  | 'rejected_by_aegis'
  | 'apply_failed'
  | 'applied'
...
export interface LLMTransmuterOptions {
  /** Répertoire de base pour résoudre les chemins relatifs. */
  basePath?: string;
  /** Profondeur de self-modification courante (passée à MetamorphicCore). */
  currentDepth?: number;
  /** Stub pour MetamorphicCore.lockCheck (utile en tests). */
  isMetamorphicCoreLocked?: () => boolean;
  /** Délai d'incubation réduit (pour les tests, default 30s). */
  incubationMs?: number;
}
...
export class LLMTransmuter extends EventEmitter {
  ...
  private aegis: Aegis;
  private metamorph: MetamorphicCore;
  private budget: TransmutationBudget;
  private options: Required<LLMTransmuterOptions>;
  private transmutationCount = 0;

  constructor(
    llm: ILLMClient,
    aegis: Aegis,
    metamorph: MetamorphicCore,
    budget: TransmutationBudget = new TransmutationBudget(),
    options: LLMTransmuterOptions = {}
  ) {
    super();
    this.llm = llm;
    this.aegis = aegis;
  ...
  public async transmutate(
    ...
    const proposal = this.metamorph.proposeTransmutation(
      gene,
      intent,
      proposedCode,
      0.5  // currentPhi fictif
    );
   ...
   */
  public async transmutateAll(
    intents: Array<{ filePath: string; intent: string }>
  ): Promise<TransmutationResult[]> {
    const results: TransmutationResult[] = [];
   ...
   */
  public buildPrompt(code: string, intent: string, filePath: string): string {
...
- If the intent is unclear or the code cannot be safely improved, return "// NO_CHANGE" on the first line.`;
   ...
   */
  private ensureModuleRegistered(absPath: string): ArchitecturalGene {
  ...
  public getStats(): {
    ...
    budget: ReturnType<TransmutationBudget['getStats']>;
  } {
    return {
