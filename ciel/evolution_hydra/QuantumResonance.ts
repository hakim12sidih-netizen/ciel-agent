...
export class QuantumResonance {
  public resolve(proposals: DecisionWave[]): { bestSolution: string; coherence: number } {
    logger.info(`[Quantum Resonance] Resolving ${proposals.length} proposal waves.`);

    const solutionGroups: Record<string, { totalAmplitude: number; interference: number; content: string }> = {};
    for (const wave of proposals) {
      ...
      for (const other of proposals) {
        if (other.cloneId === wave.cloneId) continue;
