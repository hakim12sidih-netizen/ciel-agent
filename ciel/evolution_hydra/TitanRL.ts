...
import { TorchRLBridge, type RLStepResult, type TorchRLBridgeOptions } from './TorchRLBridge.js';
...
export class TitanRL {
  private rewardWeights: number[] = new Array(12).fill(1 / 12);
  private bridge: TorchRLBridge | null = null;
  private useBridge: boolean = false;

  constructor(bridgeOptions?: TorchRLBridgeOptions) {
   ...
   */
  public compute12DReward(result: TaskResult): number[] {
   ...
   */
  public async learn(genome: { fitnessHistory: number[]; id?: string }, taskResult: TaskResult): Promise<number> {
    const rewardVector = this.compute12DReward(taskResult);

    let totalFitness: number;
   ...
   */
  public learnSync(genome: { fitnessHistory: number[] }, taskResult: TaskResult): number {
    if (this.useBridge) {
   ...
   */
  private computeStateVector(genome: { fitnessHistory: number[] }, taskResult: TaskResult): number[] {
   ...
   */
  private fitnessFromAction(action: number[] | undefined, reward: number[]): number {
    if (!action || action.length !== 12) return 0;
    let total = 0;
    for (let i = 0; i < 12; i++) {
   ...
   */
  private legacyFitness(rewardVector: number[]): number {
    let total = 0;
    for (let i = 0; i < 12; i++) {
  ...
  private adaptRewardWeights(recentHistory: number[]) {
   ...
   */
  public getBridge(): TorchRLBridge | null {
    return this.bridge;
  }

  public isUsingBridge(): boolean {
...
export class HierarchicalEpisodicMemory {
  public micro: any[] = [];    // 100ms-1s
  public mini: any[] = [];     // 1s-10s
  public mega: any[] = [];     // 10s-5min
  public epic: any[] = [];     // 5min+

  constructor() {
    logger.info('[MEH] 🧠 Hippocampe Artificiel initialisé.');
  }

  public storeMicro(state: unknown, action: unknown, reward: number[], nextState: unknown) {
    this.micro.push({
      state, action, reward, next: nextState, timestamp: Date.now()
    });
    if (this.micro.length > 10000) this.micro.shift(); // Max 10000
  }
  ...
  public consolidate() {
    ...
    if (this.micro.length > 100) {
      ...
      this.micro = [];
      if (this.mini.length > 5000) this.mini.shift();
    }
    ...
    if (this.mini.length > 50) {
      ...
      this.mini = [];
      if (this.mega.length > 1000) this.mega.shift();
    }
    ...
    if (this.mega.length > 20) {
      ...
      this.mega = [];
      if (this.epic.length > 100) this.epic.shift();
    }
  }
}
