...
export interface BudgetConfig {
  /** Max transmutations par fenêtre de temps. */
  maxTransmutations: number;
  /** Taille de la fenêtre en ms (default: 1h). */
  windowMs: number;
  /** Profondeur max d'auto-modification (default: 3). */
  maxDepth: number;
  /** Cooldown entre 2 transmutations du même fichier (ms). */
  perFileCooldownMs: number;
  /** Whitelist de préfixes de fichiers modifiables. */
  allowedPaths: string[];
  /** Blacklist de fichiers STRICTEMENT INTERDITS (override la whitelist). */
...
export type BudgetDenialReason =
  ...
  | 'COOLDOWN'
  | 'DEPTH_EXCEEDED'
  | 'METAMORPHIC_CORE_LOCKED';

export interface BudgetCheck {
  allowed: boolean;
  reason?: BudgetDenialReason;
  message?: string;
  /** Combien de transmutations dans la fenêtre actuelle. */
  currentCount?: number;
  /** Temps restant avant la prochaine transmutation possible. */
  cooldownRemainingMs?: number;
}

export class TransmutationBudget {
  private config: BudgetConfig;
  private timestamps: number[] = [];           // timestamps dans la fenêtre
  private perFileTimestamps: Map<string, number> = new Map();
  ...
  public check(
    filePath: string,
    options: { depth?: number; lockCheck?: () => boolean } = {}
  ): BudgetCheck {
   ...
   */
  public recordTransmutation(filePath: string): void {
    const now = Date.now();
  ...
  private purgeOldTimestamps(): void {
