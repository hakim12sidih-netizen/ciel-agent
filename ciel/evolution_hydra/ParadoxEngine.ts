...
import type { StrangeLoop } from './StrangeLoop.js';
...
export interface Paradox {
  id: string;
  type: ParadoxType;
  statement: string;             // L'énoncé du paradoxe
  contradiction: string;         // La contradiction spécifique
  intensity: number;             // Intensité du paradoxe (0-1)
  isGenerative: boolean;         // Si le paradoxe est génératif (productif)
  discoveredAt: number;
  resolutionAttempted: boolean;
  resolutionMethod: ParadoxResolution | null;
  transformedInto: string | null; // Ce que le paradoxe est devenu
}

export enum ParadoxResolution {
  TRANSCENDENCE = 'TRANSCENDENCE', // Créer un nouveau cadre
  DISSOLUTION = 'DISSOLUTION',     // Montrer que c'est un faux paradoxe
  EMBRACE = 'EMBRACE',             // Accepter et utiliser la contradiction
  META_LEVEL = 'META_LEVEL',       // Passer au méta-niveau
  SPLIT = 'SPLIT',                 // Séparer les contextes
}

export interface ParadoxicalInsight {
  id: string;
  fromParadox: string;
  insight: string;
  complexityGain: number;        // Gain de complexité (le paradoxe enrichit)
  phiImpact: number;             // Impact sur Φ
  newFramework: string;          // Nouveau cadre conceptuel
  isRevolutionary: boolean;      // Si l'insight change fondamentalement le système
}
...
export interface ParadoxState {
  ...
  generativeParadoxes: number;   // Paradoxes qui produisent de la nouveauté
  averageIntensity: number;
  complexityGain: number;        // Complexité gagnée grâce aux paradoxes
  highestParadox: ParadoxType;   // Type de paradoxe le plus fréquent
}
...
export class ParadoxEngine {
  ...
  private insights: ParadoxicalInsight[] = [];
  private phiEngine: PhiEngine;
  private strangeLoop: StrangeLoop | null = null;
  private state: ParadoxState;

  constructor(phiEngine: PhiEngine, strangeLoop?: StrangeLoop) {
  ...
  detectParadoxes(): Paradox[] {
    ...
    for (const p of newParadoxes) {
      ...
      if (p.isGenerative) this.state.generativeParadoxes++;
  ...
  private checkSelfReferenceParadox(phi: number): Paradox | null {
  ...
  private checkMeasurementParadox(phi: number, freeEnergy: number): Paradox | null {
  ...
  private checkInfinityParadox(depth: number, resonance: number): Paradox | null {
  ...
  private checkIdentityParadox(): Paradox | null {
  ...
  private checkCompletenessParadox(causalClosure: number): Paradox | null {
   ...
   */
  resolveParadox(paradoxId: string): ParadoxicalInsight | null {
    const paradox = this.paradoxes.get(paradoxId);
    if (!paradox || paradox.resolutionAttempted) return null;
  ...
  private selectResolutionMethod(paradox: Paradox): ParadoxResolution {
  ...
  private applyResolution(paradox: Paradox, method: ParadoxResolution): ParadoxicalInsight | null {
    const phi = this.phiEngine.getStatus().phi;

    const resolutions: Record<ParadoxResolution, () => ParadoxicalInsight> = {
   ...
   */
  runParadoxCycle(): ParadoxicalInsight[] {
    ...
    const newInsights: ParadoxicalInsight[] = [];
    ...
    const typeCounts: Record<ParadoxType, number> = {
    ...
    for (const [type, count] of Object.entries(typeCounts)) {
      if (count > maxCount) {
        maxCount = count;
        this.state.highestParadox = type as ParadoxType;
      }
    }
