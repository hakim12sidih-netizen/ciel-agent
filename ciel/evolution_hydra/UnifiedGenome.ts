...
export enum ChromosomeType {
  STRUCT = 'STRUCT',     // Architecture
  BEHAVIOR = 'BEHAVIOR', // Comportement
  EPI = 'EPI',           // Épigénétique
  META = 'META',         // Métacode
}
...
export class Gene {
  public id: number;
  public value: number;
  public chromosome: ChromosomeType;
  public mutable: boolean;

  // Marqueurs épigénétiques (hérités de V1 + V3)
  public h3k4me3: number;        // Activation (0-1)
  public h3k27me3: number;       // Répression (0-1)
  public dna_methylation: number; // Méthylation (0-1)
  ...
  public express(): number {
    if (this.h3k27me3 > 0.7) return this.value * 0.1;
    if (this.h3k4me3 > 0.7) return this.value * 1.5;
...
export interface ShadowProfile {
  repressedTemperature: number;
  repressedTools: string[];
  integrationLevel: number;  // 0 = refoulé, 1 = intégré
  valence: number;           // -1 = destructrice, +1 = créatrice
}
...
export interface UnifiedGenomeParams {
  // V1 (conservé)
  temperature: number;
  topP: number;
  ...
  dockerAffinity: number;
  polyglotDepth: number;
  // V2
  resonanceFrequency: number;
  shadowTolerance: number;
  karmicWeight: number;
  epigeneticSensitivity: number;
  metamorphicPotential: number;
  voidAttraction: number;
}

export interface UnifiedGenomeOptions {
  agentName?: string;
  generation?: number;
  params?: Partial<UnifiedGenomeParams>;
  /** Si true, utilise les tailles Titan (4096+8192+2048+4096 = 18 432 gènes). */
  titanSize?: boolean;
  /** Si true, n'initialise pas les gènes (utile pour deserialize). */
  noGenesis?: boolean;
  /** Fitness initiale (défaut 0). */
  initialFitness?: number;
  /** Lignée ancestrale (IDs). */
  ancestorIds?: string[];
  /** Si true, le mode legacy V1 est utilisé (sans V2 features). */
  legacyV1?: boolean;
}
...
export class UnifiedGenome {
  ...
  public dnaHash: string = '';

  // Fitness
  public fitness: number = 0;
  public fitnessHistory: number[] = [];
  ...
  public params: UnifiedGenomeParams;

  // 4 Chromosomes (toujours présents, taille variable)
  public g_struct: Gene[] = [];
  public g_behavior: Gene[] = [];
  public g_epi: Gene[] = [];
  public g_meta: Gene[] = [];

  // V2 : épigénétique
  public epigenome: EpigeneticMarker[] = [];

  // V2 : karma
  public karmicLedger: KarmicRecord[] = [];

  // V2 : ombre (jungienne)
  public shadow: ShadowProfile;

  // V2 : résonance
  public resonance: ResonanceSignature;

  // V2 : méta-instructions
  public metaInstructions: string[] = [];
  public ancestorIds: string[] = [];
  public isIndividuated: boolean = false;

  // Configuration interne
  private _titanSize: boolean;
  private _legacyV1: boolean;

  // ──────────────────────────────────────────────────────────
  // CONSTRUCTEUR
  // ──────────────────────────────────────────────────────────

  constructor(options: UnifiedGenomeOptions = {}) {
  ...
  private makeDefaultParams(overrides?: Partial<UnifiedGenomeParams>): UnifiedGenomeParams {
    const defaults: UnifiedGenomeParams = {
      // V1
      temperature: 0.7,
      topP: 0.9,
      promptMutation: '',
  ...
  private initializeEpigenome(): EpigeneticMarker[] {
    return [
  ...
  public updatePhenotype(): void {
    ...
    this.specialties = [];
    if (creativity > 0.6) this.specialties.push('Creative Thinking');
    if (prudence > 0.6) this.specialties.push('Rigorous Verification');
    if ((this.g_struct[0]?.express() ?? 0) > 0.5) this.specialties.push('Deep Architecture');
   ...
   */
  public getPhenotype(): Record<string, number> {
    const sampleSize = Math.min(50, this.g_struct.length);
    let epiSum = 0;
    for (let i = 0; i < sampleSize; i++) epiSum += this.g_struct[i].express();
  ...
  public express(chromosome: ChromosomeType, geneId: number): number {
    const gene = this.getGene(chromosome, geneId);
    return gene ? gene.express() : 0;
  }

  private getGene(chromosome: ChromosomeType, geneId: number): Gene | null {
    const arr = this.getChromosome(chromosome);
    return arr[geneId] ?? null;
  }

  private getChromosome(chromosome: ChromosomeType): Gene[] {
    switch (chromosome) {
   ...
   */
  public mutate(intensity = 0.1): void {
    const mutateGene = (gene: Gene) => {
   ...
   */
  public mutateV2(rate: number = 0.1): void {
    if (this._legacyV1) {
      this.mutate(rate);
      return;
    }
    ...
    if (entropy() < rate) {
      this.params.temperature = Math.max(0, Math.min(2.0,
        this.params.temperature + (entropy() * 0.3 - 0.15)));
      mutated = true;
    }
    if (entropy() < rate) {
      this.params.resonanceFrequency = Math.max(0.1, Math.min(10,
        this.params.resonanceFrequency + (entropy() * 2 - 1)));
      mutated = true;
    }
    if (entropy() < rate) {
      this.params.shadowTolerance = Math.max(0, Math.min(1,
        this.params.shadowTolerance + (entropy() * 0.2 - 0.1)));
      mutated = true;
    }
    if (entropy() < rate) {
      this.params.voidAttraction = Math.max(0, Math.min(1,
        this.params.voidAttraction + (entropy() * 0.2 - 0.1)));
      mutated = true;
    }
    if (entropy() < rate) {
      this.params.metamorphicPotential = Math.max(0, Math.min(1,
        this.params.metamorphicPotential + (entropy() * 0.2 - 0.1)));
      mutated = true;
    }
    ...
    if (entropy() < rate * 0.3) {
      ...
      this.resonance.amplitude = Math.max(0.1, Math.min(1.0,
        this.resonance.amplitude + (Math.random() * 0.2 - 0.1)));
      mutated = true;
    }
    if (mutated) {
      this.mutationCount++;
   ...
   */
  public async mutateByStrategy(
    targetGeneId: number,
    chrom: ChromosomeType,
    strategy: 'gaussian' | 'epigenetic' | 'transposon' | 'bricolage' | 'sacrifice',
    geneBank?: { resurrectGene: (c: ChromosomeType, id: number) => Gene | null }
  ): Promise<UnifiedGenome> {
    const genes = this.getChromosome(chrom);
    const targetGene = genes[targetGeneId];
    if (!targetGene) return this;
    ...
    switch (strategy) {
      ...
      case 'bricolage':
        if (geneBank) {
          ...
          if (extinct) newValue = extinct.value;
        }
        break;
    }
   ...
   */
  public fuseWith(other: UnifiedGenome): UnifiedGenome {
  ...
  public recordKarma(action: string, outcome: 'SUCCESS' | 'FAILURE' | 'NEUTRAL', impact: number, lesson: string): void {
  ...
  public consultKarma(proposedAction: string): { bias: number; wisdom: string } {
    const relevant = this.karmicLedger.filter(k =>
      k.action.toLowerCase().includes(proposedAction.toLowerCase().split(' ')[0] ?? '')
    );
  ...
  public applyEpigeneticTrigger(trigger: string): void {
  ...
  public computeResonanceWith(other: UnifiedGenome): number {
  ...
  private updateShadowProfile(): void {
    if (this._legacyV1) return;
  ...
  public static deserialize(json: string): UnifiedGenome {
   ...
   */
  public fitnessScore(): number {
    if (this.fitnessHistory.length === 0) return this.fitness || 0.5;
