...
export class MCPHost {
    ...
    async connectToServer(name, config) {
        ...
        const env = Object.fromEntries(Object.entries({ ...process.env, ...(config.env || {}) })
        ...
        const client = new Client({ name: 'HYDRA-Host', version: '1.0.0' }, { capabilities: {} });
    ...
    wrapMCPTool(serverName, mcpTool) {
        const host = this;
        return {
            ...
            execute: async (args, _context) => {
                ...
                if (!client)
                    throw new Error(`MCP Server ${serverName} disconnected.`);
