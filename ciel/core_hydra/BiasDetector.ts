...
export interface BiasAnalysis {
  ...
  overallRiskScore: number; // 0-1, how much bias affects conclusion
...
export class BiasDetector {
  ...
  detectAvailabilityBias(input: string, frequency?: { actual: number; perceived: number }): BiasDetection {
  ...
  detectConfirmationBias(
    beliefToTest: string,
    evidencePresented: { supporting: string[]; contradicting: string[] }
  ): BiasDetection {
  ...
  detectBaseRateFallacy(
    specificInfo: string,
    baseRate: number,
    likelihoods: { ifTrue: number; ifFalse: number }
  ): BiasDetection {
  ...
  detectAnchoringBias(
    firstNumberMentioned: number,
    finalEstimate: number,
    rangeExpected: { min: number; max: number }
  ): BiasDetection {
  ...
  detectTexasSharpshooterBias(
    hypothesisFormation: 'before' | 'after',
    dataAvailableWhen: 'before' | 'after'
  ): BiasDetection {
  ...
  detectConjunctionFallacy(
    singleEvent: string,
    conjunctiveEvent: string,
    probabilitiesClaimed: { single: number; conjunction: number }
  ): BiasDetection {
  ...
  detectRepresentativenessBias(
    description: string,
    categoryAssigned: string,
    baseRateOfCategory: number
  ): BiasDetection {
  ...
  detectAffectHeuristic(
    emotionalTone: 'positive' | 'negative' | 'neutral',
    rationalAssessment: 'safe' | 'risky',
    decisionMade: string
  ): BiasDetection {
    const emotionDrivesBias = 
      (emotionalTone === 'positive' && rationalAssessment === 'risky' && decisionMade === 'accept') ||
  ...
  analyzeForBias(input: string, context?: {
    baseRate?: number;
    anchor?: number;
