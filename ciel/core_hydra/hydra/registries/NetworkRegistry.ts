...
export class NetworkRegistry {
  public getTools(): ToolDefinition[] {
    return [
      tool(
        'scout_network',
        'Analyze the local network inventory and trusted lab devices.',
        { target_ip: { type: 'string' } }
      ),
      tool(
        'broadcast_telegram',
        'Send a status report through the configured Telegram bridge.',
        { message: { type: 'string' } },
        ['message']
      ),
      tool(
        'n8n_manager',
        'Trigger n8n webhooks or inspect configured n8n workflows.',
        {
