...
export class HiveServer {
  ...
  private pavions: Map<string, PavionConnection> = new Map();
  private pendingRequests: Map<string, { resolve: (data: string) => void, reject: (err: unknown) => void }> = new Map();

  constructor(private port: number = 8089) { }
  ...
  async sendCommand(pavionId: string, command: string): Promise<string> {
    const p = this.pavions.get(pavionId);
